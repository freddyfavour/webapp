...
const DashboardLayout = ({ children }) => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#FEFFF1]">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-primaryColor text-white transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${
            ...
            {navigationItems.map((item) => {
              ...
              return (
                <li key={item.path}>
                  <Link
                    to={item.path}
                    className={`flex items-center px-4 py-3 space-x-3 rounded-lg transition-all duration-200 group ${
      ...
      <div className="flex flex-col flex-1 w-full overflow-hidden">
        {/* Top Navigation */}
        <header className="z-30 flex items-center justify-between h-16 px-4 bg-white shadow-sm lg:px-6">
        ...
        {/* Page Content */}
        <main className="flex-1 overflow-y-auto px-4 lg:p-6">{children}</main>
      </div>
